...
public class PortfolioDatabaseAiController {
    ...
    public ResponseEntity<String> generatePortfolioDatabase(Long userId, @PathVariable Long portfolioId, @PathVariable Long functionId) {
        ...
        Message prompt1 = new SystemMessage("""
                ...
                너는 프로젝트 기능을 입력으로 받으면 그 프로젝트 기능에 해당하는 DB 스키마를 생성해 쥐야 돼.
                예를 들어서 입력으로
                ...
                이라고 입력 받으면 해당 기능을 구현하기 위한 DB 스키마를 생성해야돼
                먼저 필요한 Entity 들을 생각하고 Relation을 생각한 뒤,
                필요한 DB 스키마를 SQL문(create table)으로 생성해줘
                attribute가 어떤 attribute인지 주석을 달아줘야돼
                ...
                db 정보의 갯수 (schema, description)의 쌍은 너가 필요하다고 생각되는 만큼만 작성해줘
                다른 정보들은 필요 없고 table 정보만 있으면 돼
...
//        먼저 필요한 Entity를 유추한 뒤 해당 Entity에 대한 DB 스키마를 생성해줘
//        CREATE TABLE 문과 그 Entity (DB)에 대한 설명 (주석)으로 생성해줘
//        최대한 자세하게 생성하고 알려줘야돼
//
